...
func _set_last_input_type(__last_input_type, __last_controller):
	...
	if forceControllerId != -1:
		_last_controller = forceControllerId
	
	if !inputTypeChangeLockout:
		inputTypeChangeLockout = true
...
var inputTypeChangeLockout = false


func _physics_process(delta: float) -> void:
	inputTypeChangeLockout = false
...
func _convert_event_to_path(event: InputEvent):
	if event is InputEventKey:
		...
		return _convert_joypad_motion_to_path(event.axis, event.device,event.axis_value)
...
func _convert_joypad_motion_to_path(axis: int, controller: int,direction):
	var path : String = ""
	
	if axis == JOY_AXIS_LEFT_X and direction == -1:
		path = "joypad/l_stick_left"
	elif axis == JOY_AXIS_LEFT_X and direction == 1:
		path = "joypad/l_stick_right"
	elif axis ==JOY_AXIS_LEFT_Y and direction == -1:
		path = "joypad/l_stick_up"
	elif axis ==JOY_AXIS_LEFT_Y and direction == 1:
		path = "joypad/l_stick_down"
	elif axis == JOY_AXIS_RIGHT_X and direction == 1:
		path = "joypad/r_stick_right"
	elif axis == JOY_AXIS_RIGHT_X and direction == -1:
		path = "joypad/r_stick_left"
	elif axis == JOY_AXIS_RIGHT_Y  and direction == -1:
