...
func shootBullets():
	for i in bulletPerShot:
		...
		if curfirstShotAccuracy <= 0:
			var randX = rand_range(spread.x,-spread.x)#we are picking a random radius
			var randY = rand_range(spread.y,-spread.y)
			var randAngle = rand_range(0,2*PI)
					
			res = Vector2(cos(randAngle),sin(randAngle))#we are picking ar random angle on the unit circle
			res.x *= randX
			res.y *= randY
			
		curfirstShotAccuracy = max(curfirstShotAccuracy-1,0)
		cast.cast_to.x = res.x
		cast.cast_to.y = res.y
		...
		if projectile == null:
			...
			var p : KinematicBody= load(projectile).instance()
			var par = self
